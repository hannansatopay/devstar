<script lang="ts">
	...
	if(selectedImage){
		...
		doc.addImage(imgData, 'JPEG', 135, 40, 50, 50);
	}
	doc.line(20, 120, 193, 120);
	doc.setFont(undefined, 'bold');
	doc.text(20, 127, 'DESCRIPTION');
	doc.text(120, 127, 'RATE');
	doc.text(145, 127, 'QTY');
	doc.text(165, 127, 'AMOUNT');
	doc.setFont(undefined, 'normal');
	doc.line(20, 130, 193, 130);
	doc.setLineWidth(0.5);
	doc.setDrawColor(0, 0, 0);
	doc.line(15, 275, 190, 275); 
	doc.text(55, 285, '</Dev>star_Invoice-Generator - Copyright-2023');
	let y=140;
	let subtotal =0;
	let tax =0;
