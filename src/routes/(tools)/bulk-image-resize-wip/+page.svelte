<script>
	...
	//activeDiv show first features
	let activeDiv = 'Percentage';

	//percentage page veriable
	...
	//download page variable
	let fileSizeKbAfter = 0;
	let fileSizeKbBefor = 0;

	//when isvalue is true than show a features page
	let isvlue = false;

	//when imagesSelected paga false than , disable a image choose page
	let imageSelected = true;

	//  Use padding to avoid stretching or squashing images.
	let isCheck = false;

	//when isDownload is true than show a download page
	...
	//foe selectes a image formate first
	let selectedFormat = 'jpeg';

	//images formate array
	const formats = ['jpeg', 'png', 'webp'];

	//this function use when change a image file like Jped, png ,webp
	...
	//======= Function to switch the active div features ================
	...
	//for provdes a data url use in download zip file
...
<!-- after choose image show this image resize page  -->
...
{#if isvlue}
	...
	<div class="row">
		<div class="column">
			...
			<nav>
				<ul>
					<button class="Choose_file" on:click={() => switchDiv('Percentage')}>Percentage</button
					><br />
					<!-- <button class="Choose_file" on:click={() => switchDiv('FileSize')}>File Size</button><br
					/> -->
					<button class="Choose_file" on:click={() => switchDiv('ImageDimensions')}
						>Image Dimensions</button
					><br />
					<button class="Choose_file" on:click={() => switchDiv('WidthDiv')}>Width</button><br />
					<button class="Choose_file" on:click={() => switchDiv('HeightDiv')}> Height</button><br />
					<button class="Choose_file" on:click={() => switchDiv('LongestSide')}>Longest Side</button
					>
					<div class="row">
						<div class="column2">
							<button class="Choose_file1" on:click={() => backMenu()}>Back</button>
						</div>
						<div class="column2">
							<button class="Choose_file1" on:click={() => deltetImage()}>Delete</button>
						</div>
					</div>
				</ul>
			</nav>
			...
			<div id="ImageDimensions">
				...
				<div class="column1">
					<p class="p1">
						...
						/>
						x
						<input class="input1" type="number" bind:value={resizeDimensionsHeight} min="1" /> (Width
						× Height)
					</p>
			...
			<div id="LongestSide">
				...
				<div class="column1">
					...
					{#if selectedImage}
						<center>
							<!-- <img   src={selectedImage} alt="Selected Image" /> -->
							<button type="button" class="bn" on:click={longSideFunction}>Start</button>
						</center>
					{/if}
				</div>
			</div>
...
{#if isDownload}
	...
	<center>
		...
		{#if activeDiv == 'Percentage'}
			<button type="button" class="download" on:click={percentaFunction}>Download </button>
		{/if}

		{#if activeDiv == 'ImageDimensions'}
			<button type="button" class="download" on:click={imageDimFunctioin}>Download </button>
		{/if}

		{#if activeDiv == 'WidthDiv'}
			<button type="button" class="download" on:click={widthFunction}>Download </button>
		{/if}

		{#if activeDiv == 'HeightDiv'}
			<button type="button" class="download" on:click={heightFunction}>Download </button>
		{/if}

		{#if activeDiv == 'LongestSide'}
			<button type="button" class="download" on:click={longSideFunction}>Download </button>
		{/if}
		<div class="rowdown">
			<div>
