...
<Intro heading="Upload PDF" description="Select a PDF file and enter the password to decrypt" />
<main class="flex min-h-screen justify-center font-sans">
  <section class="bg-white dark:bg-gray-900" style="color: azure;" for="dropzone-file">
    <label class="mx-auto cursor-pointer flex w-full max-w-lg flex-col items-center rounded-xl border-2 border-dashed border-blue-400 bg-white p-6 text-center">
      ...
      <p class="mt-2 text-gray-500 tracking-wide">Select a PDF file and enter the password to decrypt</p>

      <input id="dropzone-file" type="file" class="hidden" accept=".pdf" on:change={handleFile} />
      <input type="text" class="border rounded mt-3 w-full text-gray-800" placeholder="Password" on:input={handlePass} />
