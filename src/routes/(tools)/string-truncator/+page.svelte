<script lang="ts">
    ...
    function truncateString(input) {
        if (input.target.value.length > characterLimit) 
		{
		 truncatedText = input.value.slice(0, characterLimit-1) + '';
		}
        else 
		{
            truncatedText = input.target.value;
        }
	}
    ...
    function downloadText() {
        if (truncatedText.length > 0) {
            var filename = "TruncatedText.txt";
            var blob = new Blob([truncatedText], { type: 'text/plain' });
    ...
    function downloadPDF() {
        if (truncatedText.length > 0) {
            const doc = new jsPDF();
            doc.text(truncatedText, 20, 20);
            doc.save('TruncatedText.pdf');
        }
    }
</script>
...
<section class="bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-screen-xl lg:px-12">
        <div class="card p-8 relative items-center mx-auto max-w-screen-xl overflow-hidden rounded-lg">
            ...
            <div>
                <label for="" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Character Limit</label>
